...
<#assign _description = content.description!
    ((content.type == "post" || content.type == "publication")?then(content.body?replace("<[\\w/][^>]*>", "", "r")?replace("\\s+", " ", "r")?truncate(200, "...")?trim,
...
<head>
    ...
    <meta content="${menu.project}" property="og:site_name">
    <meta content="${(content.type == "post")?then("article", "website")}" property="og:type">
    <meta content="${config.canonicalBaseUrl + "/" + ((content.social_media_share_image??)?then(_uri?replace("/[^/]*$", "/", "r") + content.social_media_share_image, "headerFooter/profile.png"))}" property="og:image">
    <meta content="summary" name="twitter:card">
    <meta content="@${menu.project}" name="twitter:site">
    ...
    <link rel="stylesheet" href="${content.rootpath}${menu.css}">
...
<body>
    ...
    <script src="${content.rootpath}${menu.js}"></script>
    <script>
